...
require '/home/sammysatoro/RubyProjects/Ruby_Labs/App/Controllers/LaboratoryWork/update_lw_controller.rb'
...
class UpdateLWView
  ...
  attr_reader :number_label, :update_window

  def initialize(main_view, senior_controller)
    @controller = UpdateLWController.new(self, main_view, senior_controller)
  ...
  def display
    @update_window = window('Добавить лабораторную работу', 800, 400) {
      margined true
      grid {
  ...
  def on_clicked_button
    cur_date = @date_of_issue_entry.text.force_encoding('UTF-8')
    message = @controller.find_case(cur_date)
    unless message
      input_scan
    else
